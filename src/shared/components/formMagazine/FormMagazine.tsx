...
//             <Image src={imagePreview} alt="Vista previa" className="w-32 h-32 object-cover border rounded-md" />
//           </div>
//         )}
...
export default function FormMagazine() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (selectedId) {
        await axios.put(`${API_BACK}/api/magazine/${selectedId}`, formDataToSend);
        console.log("Artículo actualizado:", formDataToSend);
      } else {
  ...
  const handleDelete = async (id: string) => {
    try {
      await axios.delete(`${API_BACK}/api/magazine/${id}`);
      console.log("Artículo eliminado con éxito.");
      setArticles(articles.filter((article) => article.id !== id));
      setFormData({ author: "", title: "", description: "", image: "" });
      setSelectedId(null);
      ...
      if ((error)) {
        ...
        console.error("Error desconocido al eliminar el artículo.");
      }
    }
  };
        ...
        <div className="flex gap-3 mt-3">
          <button
            ...
            className={`px-4 py-2 rounded text-white transition ${
              selectedId ? "bg-green-500 hover:bg-green-600" : "bg-blue-500 hover:bg-blue-600"
            }`}
          >
            {selectedId ? "Actualizar" : "Publicar"}
          </button>
