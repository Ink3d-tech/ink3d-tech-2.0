...
export const CartProvider = ({ children }: { children: React.ReactNode }) => {
    ...
    const addProductToCart = (product: Product) => {
        ...
        setProducts((prev) => {
            const updatedCart = prev.some(p => p.id === product.id)
                ? prev.map(p => p.id === product.id ? { ...p, units: p.units + 1 } : p)
                : [...prev, { ...product, units: 1 }];
            saveCart(updatedCart);
            return updatedCart;
        });
    ...
    const countProducts = () => products.reduce((sum, product) => sum + product.units, 0);

    const handleProductIncrease = (id: string, stock: number) => {
        setProducts((prev) => {
            const updatedProducts = prev.map(product =>
                product.id === id && product.units < stock
