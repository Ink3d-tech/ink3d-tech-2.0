...
export const CategoriesProvider = ({ children }: { children: React.ReactNode }) => {
    ...
    const createCategory = async (category: CategoryInterface) => {
        try {
            const res = await axios.post<CategoryInterface>(`${API_BACK}/categories`, category, getAuthHeaders());
            setCategories(prev => [...prev, res.data]);
        } catch (error) {
            Mixin.fire("Error al crear la categoría", "", "error");
    ...
    const getCategoryById = async (categoryId: string) => {
        try {
            await axios.get<CategoryInterface>(`${API_BACK}/categories/${categoryId}`, getAuthHeaders());
        } catch (error) {
            setError(error instanceof Error ? error.message : "Error al obtener la categoría");
        }
    };

    const deleteCategory = async (id: string) => {
        try {
            await axios.delete(`${API_BACK}/categories/${id}`, getAuthHeaders());
            setCategories(prev => prev.filter(category => category.id !== id));
        } catch (error) {
            Mixin.fire("Error al eliminar la categoría", "", "error");
